export default class Mortgage {
  ...
  equalPayments() {
    ...
    for (let month = 1; month <= this.length; month++) {
      ...
      this.principal_history[month] =
        this.principal_history[month - 1] * (1 + this.interest_rate) - m;
  ...
  equalPaymentCpi() {
    ...
    for (let month = 1; month <= this.length; month++) {
      ...
      this.installment_history[month] =
        Math.pow(1 + this.inflation, month) * m - this.interest_history[month];
    }
  }
  ...
  getPlotableRollingPayments() {
    ...
    for (let i = 1; i < this.interest_history.length; i++) {
      data[i] =
        this.interest_history[i] + this.installment_history[i] + data[i - 1];
    }
    return data;
  }
}
